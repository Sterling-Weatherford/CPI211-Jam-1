...
public class Ladder : MonoBehaviour
{
	...
	//what to do if we are in contact we a ladder object for more than a second
		...
		if (other.GetComponent<LadderType> ()) {
			if (verticalInput > 0.1) {
				if (other.GetComponent<LadderType> ().isLadder) {
					Vector3 tempVector;
					//csharp doesn't let you modify a transform's x,y or z position by iself so you have to write out this code instead 
					tempVector = transform.position;
					tempVector.x = other.transform.position.x;
					tempVector.z = other.transform.position.z - 0.9f;
